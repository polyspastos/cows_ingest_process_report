...
def generate_report(db_session: Session, report_date: datetime):
    ...
    milk_data = db_session.query(
        MilkProduction.cow_id,
        func.sum(MilkProduction.value).label('total_milk')
    ).filter(
        func.date(MilkProduction.timestamp) == report_date.date()
    ).group_by(MilkProduction.cow_id).all()
    ...
    for cow_id, cow_name in db_session.query(Cow.id, Cow.name).all():
        latest_weight = db_session.query(Weight.value).filter(
            Weight.cow_id == cow_id
        ).order_by(Weight.timestamp.desc()).first()

        thirty_days_ago = report_date - timedelta(days=30)
        avg_weight = db_session.query(func.avg(Weight.value)).filter(
            and_(
                Weight.cow_id == cow_id,
    ...
    for cow_id, data in report_data.items():
        ...
        report += f"30-day Avg Weight: {data.get('avg_weight_last_30_days', 'N/A')} kg\n"
        report += "-----------------------------------------\n"
