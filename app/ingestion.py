    ...
    try:
        async with session.post(endpoint, json=data) as response:
            ...
            if response.status == 400:
                info_counter += 1
                logger.info(f"[{info_counter}] Sensor {sensor_id} already exists in the database")
            elif response.status == 201:
                info_counter += 1
                logger.info(f"[{info_counter}] Successfully added sensor {sensor_id}")
            else:
                response_text = await response.text()
                logger.error(f"Failed to add sensor {sensor_id}: Status {response.status}, Body: {response_text}")
...
async def process_cow(session, base_url, cow_id, name, birthdate):
    ...
    try:
        async with session.post(endpoint, json=payload) as response:
            ...
            if response.status == 400:
                info_counter += 1
                logger.info(f"[{info_counter}] Cow {cow_id} already exists in the database.")
            elif response.status == 201:
                info_counter += 1
                logger.info(f"[{info_counter}] Successfully added cow {cow_id}")
            else:
                response_text = await response.text()
                logger.error(f"Failed to add cow {cow_id}: Status {response.status}, Body: {response_text}")
                response.raise_for_status()
    except aiohttp.ClientError as e:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if pd.isna(row['value']) or row['value'] <= 0:
                logger.warning(f"Invalid value for cow {cow_id}, sensor {sensor_id}, timestamp {timestamp}: {row['value']}")
                failed_measurements += 1
                return
            ...
            async with session.post(endpoint, json=data) as response:
                ...
                if response.status == 422:
                    logger.warning(f"Unprocessable Entity for cow {cow_id}, sensor {sensor_id}, timestamp {timestamp}, data: {data}")
                    failed_measurements += 1
                    return
                elif response.status >= 400:
                    response_text = await response.text()
                    logger.warning(f"Error response for cow {cow_id}, sensor {sensor_id}, timestamp {timestamp}: Status {response.status}, Body: {response_text}")
                    raise aiohttp.ClientError(f"HTTP {response.status}: {response_text}")
                response.raise_for_status()
                info_counter += 1
                logger.info(f"[{info_counter}] Successfully added {sensor_type} data for cow {cow_id}, sensor {sensor_id}, timestamp {timestamp}")
                successful_measurements += 1
            return
        except aiohttp.ClientError as e:
            logger.warning(f"Failed to add data for cow {cow_id}, sensor {sensor_id}, timestamp {timestamp} (attempt {attempt + 1}): {str(e)}")
            if attempt < MAX_RETRIES - 1:
                await asyncio.sleep(2 ** attempt + random.random())
            else:
                logger.error(f"Failed to add data for cow {cow_id}, sensor {sensor_id}, timestamp {timestamp} after {MAX_RETRIES} attempts")
...
async def ingest_data(base_url: str):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            sensors_df = pd.read_parquet("cow_data/sensors.parquet", engine='fastparquet')
            ...
            for _, row in cows_df.iterrows():
                await process_cow(session, base_url, UUID(row['id']), row['name'], datetime.fromtimestamp(row['birthdate'] / 1_000_000_000))

            measurements_df = pd.read_parquet("cow_data/measurements.parquet", engine='fastparquet')
            logger.info(f"Read {len(measurements_df)} rows from cow_data/measurements.parquet")
        ...
        logger.info(f"Total measurements processed: {successful_measurements + failed_measurements}")
